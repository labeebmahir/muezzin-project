...
export async function shareAsImage({ prayers, hijriDate, gregorianDate, city, language }) {
  ...
  const logoY = 20
  if (logoImg) ctx.drawImage(logoImg, logoX, logoY, logoSize, logoSize * (162 / 160))
  ...
  ctx.strokeStyle = COLORS.gold + '33' // 20% opacity
  ctx.lineWidth = 1
  ctx.stroke()
  ...
  prayers.forEach((prayer, i) => {
    ...
    ctx.fillStyle = i % 2 === 0 ? COLORS.green : COLORS.green + '80' // 50% opacity alternate
    ctx.fill()
